...
export default function DocSpecialistHome() {
  ...
  const originalFlatRowsRef = useRef([]);   // ✅ FIX
  ...
  async function uploadTbDocToBackend() {
  if (!tbDocRef.current) {
  ...
  if (!docName || !clientId) {
    alert("Document Name and Client are required");
    return;
  }
  ...
  async function loadClientDocs() {
    ...
    try {
      const res = await fetch(
        `/api/trial-balance/client/${selectedClient}`
      );

      if (!res.ok) throw new Error("Failed to fetch");

      const data = await res.json();
      setTbList(data.items || []);
    } catch (e) {
      console.error(e);
      setTbList([]);
    } finally {
...
async function openTrialBalance(docId) {
  try {
    const res = await fetch(`/api/trial-balance/doc/${docId}`);
    ...
    setSelectedSheet(doc.sheetName);
    setMeta(doc.meta);
    setFlatRows(doc.rowsFlat);
    setTbDoc(doc);
...
async function saveTbDocToBackend() {
  if (!tbDocRef.current || !tbDocRef.current.fileId) {
    alert("No file ID available to save");
    setIsSaving(false);
    return;
  }

const payload = {
  fileId: tbDocRef.current.fileId,
  rowsFlat: flatRowsRef.current,
  updatedAt: new Date().toISOString(),
  updatedBy: user.username,
  clientId: selectedClient, // Ensure clientId is added here
};
  ...
  try {
    const res = await fetch(`/api/trial-balance/${tbDocRef.current.fileId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto max-w-6xl px-4 py-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-base font-semibold text-slate-900">Upload Excel</h2>
        {/* ---- Document Metadata ---- */}
<div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-black">
  <input
